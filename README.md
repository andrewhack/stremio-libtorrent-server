...
| `STREMIOSRV_LIBRARY_ADDON_ALLOW` | *(unset)* | Which client addresses count as your own network. They may reach the library addon, and `/proxy` (which plays addon streams that need their own request headers) fetches any address for them, while other clients may fetch public addresses only. Unset means loopback, private ranges, link-local, IPv6 ULA and carrier-grade NAT (so a private tunnel still works). Comma-separated CIDRs to replace that list. **Behind a reverse proxy** every client arrives from the proxy's address and so counts as local: narrow this list to exclude it. |
